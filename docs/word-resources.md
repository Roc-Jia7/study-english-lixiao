...
- [x] 资源调研 + 分类 + 许可核验(本文件)
- [ ] **(b) 样例词包验证**:手工 1–2 个示例包(新课标小学核心 / 人教三上 Unit1)→ 接现有卡片与会话,看数据质量与效果。← 进行中
- [ ] 验证通过后:写离线预处理脚本(`scripts/`)从 ECDICT + DictionaryData 产出精简分级词包 + 许可证 NOTICE。
- [ ] 计划引擎:配额驱动(每天 N 新词 + 到期复习)+ ETA + 进度环。
