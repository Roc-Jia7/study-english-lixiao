...
- **Phase 1(下一步)**:搭流水线骨架 + **跑通 1 个真实包**(端到端验证:数据质量 / 体积 / 卡片效果),替换一个手工样例。
- **Phase 2**:扩到 K12 分级集(小学/中考/高考)+ 1~2 套教材;加懒加载 + 词包选择器(教材/考试分组)。
- **Phase 3(可选)**:CSV/TXT 用户自定义导入(复用补全层 + 内置 ECDICT-mini 客户端补全)。
- **Phase 4(可选)**:课标覆盖率报告进家长看板("覆盖课标 92%")。

---

## 6. 待定决策(需你拍板,影响 Phase 1 先做什么)

1. **Phase 1 先跑哪个真实包**:贴教材(人教三上,验证 DictionaryData 链路)还是分级(小学核心/中考,验证 ECDICT∩课标链路)?
2. **现在就搭离线流水线**,还是再手工补几个样例验证体验后再上流水线?
3. **加载**:起步集内联 + 其余懒加载(推荐)。
